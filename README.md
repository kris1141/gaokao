...
## 纯静态 SPA 版本（无需后端）
- 入口文件：`index.html`（项目根目录）
- 浏览器端解析 XLS/XLSX：使用 `SheetJS (xlsx)` CDN，在本地完成读取与计算
- 支持功能：
  - 高考分预测（历史/物理/艺术）
  - 联考换算预测（历史/物理，按百分位映射）
  - 冲/稳/保分档与概率说明
...
使用方式：
1. 直接用浏览器打开 `index.html`（推荐通过本地静态服务访问，避免部分浏览器安全策略影响）。
2. 在页面先上传数据文件（历史与物理主数据必填，艺术与联考可选）。
3. 点击“加载并建立索引”后即可在 SPA 内预测。

自动拉取默认 XLS：
- 页面提供“自动拉取默认 XLS”按钮，会按项目内默认文件名直接 fetch 并加载。
- 必需文件（任一缺失会失败）：
  - `./hist.xlsx`
  - `./历史类含加分一分段表.xlsx`
  - `./phys.xlsx`
  - `./物理类含加分一分段表.xlsx`
- 可选文件：
  - 艺术：`./艺术类.xlsx` 或 `./艺术批.xlsx`
  - 联考：根目录与 `./mock_exams/` 下若干默认命名（如 `历史_一诊.xlsx`、`物理_一诊.xlsx`、`历史_康德一诊.xlsx` 等）

常见文件对应：
- 历史录取表：`hist.xlsx`
- 历史一分一段：`历史类含加分一分段表.xlsx`
- 物理录取表：`phys.xlsx`
- 物理一分一段：`物理类含加分一分段表.xlsx`
- 艺术录取表（可选）：`艺术批.xlsx` 或 `艺术类.xlsx`
- 联考（可选，多文件）：`mock_exams/*.xlsx`（文件名需能识别历史/物理）
